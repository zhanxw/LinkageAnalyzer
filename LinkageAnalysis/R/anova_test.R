  ...
  test_commands <- function() {
    ...
    if (test == "wG2") {
      if (length(unique(data$sex)) == 1) {
        if (bFitAlt ) {fit <- glmer(pt ~ gt + (1 | mother), family = fam, nAGQ = 10, data = data)}  # full model
        if (bFitNull) {fit0 <- glmer(pt ~ (1 | mother), family = fam, nAGQ = 10, data = data)}  # reduced model
      } else {
        if (bFitAlt ) {fit <- glmer(pt ~ gt + sex + (1 | mother), family = fam, nAGQ = 10, data = data)}  # full model
        if (bFitNull) {fit0 <- glmer(pt ~ sex + (1 | mother), family = fam, nAGQ = 10, data = data)}  # reduced model
      }
      ...
      if (length(unique(data$sex)) == 1) {
        if (bFitAlt ) {fit <- glm(pt ~ gt, family = fam, data = data)}  # full model
        if (bFitNull) {fit0 <- glm(pt ~ 1, family = fam, data = data)}  # reduced model
      } else {
        if (bFitAlt ) {fit <- glm(pt ~ gt + sex, family = fam, data = data)}  # full model
        if (bFitNull) {fit0 <- glm(pt ~ sex, family = fam, data = data)}  # reduced model
      }
