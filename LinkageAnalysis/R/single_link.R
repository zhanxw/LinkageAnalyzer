single_link <- function(main_file = "", G2_file = "", output = ".", test = "wG2", 
  detect = "never", silent = T, tail = "decreasing", prefix = "", n_trial = 1e+05) {
  ...
  # read data
  raw_data <- get_data(main_file, G2_file, fns$log_file, detect)
  ...
  for (type in c("additive", "recessive", "dominant", "TDT")) {
    bonferroni <- mht(genes, sig_gene, type, fns$log_file, test, bin, effective, 
      genotype)
  }
  dev.off()
