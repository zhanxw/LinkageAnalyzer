...
double_link <- function(main_file, G2_file = "", output = ".", test = "woG2", detect = "never", 
  silent = TRUE, tail = "decreasing", prefix = "", cutoff_single = 0.01, n_trial = 1e+05) {
  # input
  fns <- filename(output, prefix)  # generate output file names
  input <- get_data(main_file, G2_file, fns$log_file, detect)  # read data,G2 is null is G2 dam genotype data are not available
  ...
  for (i in 1:(input$n - 1)) {
    ...
    for (j in (i + 1):input$n) {
      ...
      for (type in c("recessive", "additive", "dominant", "inhibitory")) {
        ...
        if (type == "inhibitory") {
          data$gt <- data$gt1 * (data$gt2 == 0) + data$gt2 * (data$gt1 == 
          0)
        }
        ...
        sig[[type]][i, j] <- anova_test(data, input$bin, test, silent, fns$log_file, 
          tail)
    ...
    for (j in (i + 1):input$n) {
      ...
      for (type in c("recessive", "additive", "dominant", "inhibitory", "lethal")) {
        if (sig[[type]][i, j] < cutoff_pair && SignifOfSingle[i, type] > 
          ...
          flag <- paste(flag, " ", substr(type, 1, 2), " (", pretty_num(sig[[type]][i, 
          j]), ")", sep = "")
        }
      }
      if (flag == "") {
