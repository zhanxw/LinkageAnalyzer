...
get_genetics <- function(genes, phenotype, genotype, bin, unconverted) {
  ...
  for (i in 1:dim(genes)[1]) {
    if (bin == TRUE) {
      # penetrance
      genetics[i, "Penetrance_REF"] <- sum(genotype[i, ] == "REF" & phenotype$phenotype == 
        "AFFECTED")/sum(genotype[i, ] == "REF")
      genetics[i, "Penetrance_HET"] <- sum(genotype[i, ] == "HET" & phenotype$phenotype == 
        "AFFECTED")/sum(genotype[i, ] == "HET")
      genetics[i, "Penetrance_VAR"] <- sum(genotype[i, ] == "VAR" & phenotype$phenotype == 
        "AFFECTED")/sum(genotype[i, ] == "VAR")
    }
    
    if (bin == TRUE && is.null(unconverted)) {
      next
    }
    tmp <- (mean(pt[genotype[i, ] == "REF"]) - mean(pt[genotype[i, ] == "HET"]))/(mean(pt[genotype[i, 
      ] == "REF"]) - mean(pt[genotype[i, ] == "VAR"]))
    if (!is.na(tmp)) {
      if (tmp > 1) {
        tmp <- 1
      }
